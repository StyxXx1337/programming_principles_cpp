...
public:
  ...
  bool operator==(const Name_pairs& lhs){
    for (int i = 0; i < this->names.size(); i++){
      if (this->names[i] != lhs.names[i]){ return false; };
      if (this->ages[i] != lhs.ages[i]){ return false; };
    }
    return true;
  }
...
// Is part of exercise3 but already completed in 2.
...
int main(){
  ...
  Name_pairs another = np;
  cout << "NP is equal to another? " << (another == np) << endl;
  np.sort_by_name();
  cout << "NP is not equal to another after sorting? " << (another != np) << endl;
