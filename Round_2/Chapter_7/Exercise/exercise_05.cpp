...
// Exercise-04: Create a val_tbl class and make set, get, is_declared, declared_at methods of that class.
// TODO Exercise-05: Make Token(print) when a new line character is coming in.
...
double Var_table::get_value(string s){
	for (int i = 0; i < list.size(); ++i)
		if (list[i].name == s) return list[i].value;
...
void Var_table::update_value(int pos, double d){
	if (pos >= list.size()) error("out of range error");
	if (list[pos].type == 'C') error(list[pos].name, " is a constant");
	list[pos].value = d;
}

bool Var_table::is_declared(string s){
	for (int i = 0; i < list.size(); ++i)
		if (list[i].name == s) return true;
	return false;
}
...
{	
	...
	if (names.is_declared(name)) error(name, "> declared twice"); // Currently vars can't be updated.
	Token t2 = ts.get();
